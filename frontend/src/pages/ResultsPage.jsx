...
import MarketplaceList from '../components/Gift/MarketplaceList';
...
const ResultsPage = () => {
  ...
  const { data: marketplaceData, isLoading: marketplaceLoading } = useQuery({
    queryKey: ['marketplaces', recommendations?.map(g => g.id)],
    queryFn: () => api.getMarketplaceOffers(recommendations?.map(g => g.id)),
  ...
  const handleShare = () => {
    shareResults({
      title: 'Мои рекомендации подарков от GiftWizard',
      text: `Я нашел идеальные подарки для ${answers?.age}-летнего ${answers?.gender === 'male' ? 'мужчины' : 'женщины'} с помощью AI!`,
      ...
      <div className="mb-8">
        ...
        <div className="bg-gradient-to-r from-purple-600 to-pink-600 rounded-2xl p-8 text-white">
          ...
          <p className="text-lg opacity-90 mb-6">
            На основе ваших ответов AI подобрал лучшие варианты подарков
          </p>
          ...
          <div className="flex flex-wrap gap-4">
            ...
            >
              ...
              <span>
                {showMarketplaces ? 'Скрыть магазины' : 'Где купить рядом?'}
              </span>
            </button>
          </div>
        </div>
      ...
      {showMarketplaces && (
        <div className="mb-8">
          {marketplaceLoading ? (
            ...
            <MarketplaceList offers={marketplaceData} />
          )}
        </div>
      )}
        ...
        <div className="lg:w-3/4">
          <div className="mb-4 flex justify-between items-center">
            <p className="text-gray-600">
              Найдено <span className="font-semibold">{filteredGifts.length}</span>{' '}
              подарков
            </p>
            <select
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {filteredGifts.map((gift, index) => (
              ...
              >
                <GiftCard gift={gift} showActions />
              </motion.div>
            ))}
          </div>
