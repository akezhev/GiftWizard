...
const QuizPage = () => {
  ...
  const mutation = useMutation({
    ...
    onSuccess: async (data) => {
      // Сохраняем результат в IndexedDB для оффлайн
      await saveQuizResult({
        id: data.id,
        answers,
        recommendations: data.gifts,
        timestamp: Date.now(),
      });

      // Показываем уведомление
      showNotification('Успех!', 'Рекомендации подарков готовы!');

      // Переходим на страницу результатов
      navigate(`/results/${data.id}`, {
        state: { recommendations: data.gifts, answers },
      });
    },
    onError: (error) => {
        ...
        <div className="border-t px-6 py-4 bg-gray-50 flex justify-between">
          ...
          {step < 4 && (
            <button
              ...
              disabled={mutation.isPending}
              className="ml-auto flex items-center space-x-2 bg-purple-600 hover:bg-purple-700 text-white px-6 py-2 rounded-lg transition-colors disabled:opacity-50"
            >
              <span>Далее</span>
          ...
          {step === 4 && (
            <button
              ...
              disabled={mutation.isPending}
              className="ml-auto flex items-center space-x-2 bg-green-600 hover:bg-green-700 text-white px-6 py-2 rounded-lg transition-colors disabled:opacity-50"
            >
              {mutation.isPending ? (
                <>
